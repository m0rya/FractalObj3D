//
//  TruncatedTetrahedron.cpp
//  Koch3D
//
//  Created by m0Rya on 2017/04/25.
//
//
...
void truncatedTetrahedron::calcMesh(){
    ...
    for(int i=0; i<4; i++){
        ...
        for(int j=0; j<6; j++){
            ...
            argForHIH[j] = pointTT[i][j];
        }
        hexInHex(argForHIH, raito, heightForHIH, numRecursion);
    }
    ...
    for(int i=0; i<4; i++){
        for(int j=0; j<3; j++) mesh.addIndex(numVertices + index[i][j]);
    }
...
void truncatedTetrahedron::setRaito(float _raito){
    raito = _raito;
    calcMesh();
}

void truncatedTetrahedron::setHeightForHIH(float _height){
    heightForHIH = _height;
    calcMesh();
}

void truncatedTetrahedron::setNumRecursion(int _numRecursion){
